...
"""
    ...
    def _run_ids_analysis(self, pcap_path: str, engine: str, log_dir: Path, rules: str = None, custom_rules_file: str = None, cleanup: bool = True) -> dict:
        ...
        try:
            ...
            pcap_host_abs = os.path.abspath(pcap_path)
           
            project_root = Path(__file__).resolve().parents[2]  # ← Racine du projet SPQR
            log_debug("🔍 Project root: ", project_root)
            log_debug("🔍 Current file: ", Path(__file__).resolve())
            log_debug(Path(__file__).resolve().parents[0])
            log_debug(Path(__file__).resolve().parents[1])
            log_debug(Path(__file__).resolve().parents[2])
            log_debug(Path(__file__).resolve().parents[3])
            log_debug(Path(__file__).resolve().parents[4])
            log_debug(Path(__file__).resolve().parents[5])
            ...
            if engine_name == "suricata":
                cmd = [
                    ...
                    *volume_mounts,
                    ...
                    "-S", f"/etc/{engine_name}/{rules_filename}",
                    "-r", "/pcap/input.pcap",
