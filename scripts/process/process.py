...
"""
    ...
    def _run_ids_analysis(self, pcap_path: str, engine: str, log_dir: Path, rules: str = None, custom_rules_file: str = None, cleanup: bool = True) -> dict:
        ...
        try:
            ...
            print(f"📄 Fichier PCAP sélectionné : {pcap_host_abs}")
            print(f"📁 Répertoire temporaire utilisé : {temp_dir}")
            ...
            volume_mounts = [
            "-v", f"{pcap_host_abs}:/pcap",
            "-v", f"{rules_path}:/etc/{engine_name}/spqr_{rules_filename}",
            "-v", f"{temp_dir}/{config_name}:/etc/{engine_name}/spqr_{config_name}",
